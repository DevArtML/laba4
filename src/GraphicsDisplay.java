...
public class GraphicsDisplay extends JPanel {
    ...
    protected void paintMarkers(Graphics2D canvas, double offsetX, double offsetY) {
        ...
        for (Double[] point : graphicsData) {
            ...
            boolean result;
            if (yValue < 0){
                result = Math.ceil(yValue) != Math.ceil(yValue + 0.1) ||
                        Math.ceil(yValue) != Math.ceil(yValue - 0.1);
            }
            else {
                result = Math.floor(yValue) != Math.floor(yValue + 0.1) ||
                        Math.floor(yValue) != Math.floor(yValue - 0.1);
            }
